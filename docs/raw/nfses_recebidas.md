...
Source: https://focusnfe.com.br/doc/#nfses-recebidas
...
api_version: Se informado, devolve os dados estruturados conforme seção Versões da API. Caso contrário, adota a última versão estável.
...
status: Status do documento fiscal. Pode ser autorizado ou cancelado.
...
Isto irá devolver os mesmos campos da nota completa, conforme descritos na seção anterior.
...
Eventualmente são integradas melhorias nos dados retornados pela API de NFSes Recebidas.
Para garantir compatibilidade e facilitar transições, é possível consultar dados de versões específicas da API informando o parâmetro api_version (recomendado).
